    ...
    Layer::load(const sf::Texture& tileset, sf::Vector2u tileSize, std::unique_ptr<int[]>&& tiles, size_t width, size_t height)
    {
        ...
        for (unsigned int i = 0; i < height; ++i)
            for (unsigned int j = 0; j < width; ++j)
            {
                ...
                const gsl::span<sf::Vertex> quad(&m_vertices[(i * width + j) * 4], 4);
                
                //std::cout << quad.size() << std::endl;

                // define its 4 corners
                gsl::at(quad, 0).position = sf::Vector2f(j * tileSize.x, i * tileSize.y);
                gsl::at(quad, 1).position = sf::Vector2f((j + 1) * tileSize.x, i * tileSize.y);
                gsl::at(quad, 2).position = sf::Vector2f((j + 1) * tileSize.x, (i + 1) * tileSize.y);
                gsl::at(quad, 3).position = sf::Vector2f(j * tileSize.x, (i + 1) * tileSize.y);


                // define its 4 texture coordinates
                gsl::at(quad, 0).texCoords = sf::Vector2f(tu * tileSize.x, tv * tileSize.y);
                gsl::at(quad, 1).texCoords = sf::Vector2f((tu + 1) * tileSize.x, tv * tileSize.y);
                gsl::at(quad, 2).texCoords = sf::Vector2f((tu + 1) * tileSize.x, (tv + 1) * tileSize.y);
                gsl::at(quad, 3).texCoords = sf::Vector2f(tu * tileSize.x, (tv + 1) * tileSize.y);
            }
