...
namespace ezg {
	...
	void MushroomBlue::_setAnimations_() /*override*/ {
		...
		m_animation.addAnimation(EntityAnimation::Idle);
		m_animation.addFrame(EntityAnimation::Idle, sf::IntRect(1, 38, 6, 2), 1000000.f);

		m_animation.addAnimation(EntityAnimation::Attack);
		m_animation.addFrame(EntityAnimation::Attack, sf::IntRect(33, 38, 6, 2), 100.f);
		m_animation.addFrame(EntityAnimation::Attack, sf::IntRect(41, 37, 6, 3), 200.f);
		m_animation.addFrame(EntityAnimation::Attack, sf::IntRect(49, 35, 6, 5), 200.f);
		m_animation.addFrame(EntityAnimation::Attack, sf::IntRect(57, 34, 6, 6), 400.f);
		m_animation.addFrame(EntityAnimation::Attack, sf::IntRect(49, 35, 6, 5), 200.f);
		m_animation.addFrame(EntityAnimation::Attack, sf::IntRect(41, 37, 6, 3), 200.f);
		m_animation.addFrame(EntityAnimation::Attack, sf::IntRect(33, 38, 6, 2), 100.f);
...
#define MAX_SPEED_SNAKE 0.04f
	...
	float Snake::attack(sf::FloatRect _rec) {
		...
		if (m_effect == EntityEffect::Attack) {
	...
	void Snake::_goto_(float _x, float _y, float _time) {
		...
		if (_y < m_hit_box.top - 6) {
			jump();
		}
		else if (_y > m_hit_box.top + 6) {
			jumpOff();
		}
